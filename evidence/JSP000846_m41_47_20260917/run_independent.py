...
out=b/'results'/'independent';out.mkdir(exist_ok=True);counts={};start=time.monotonic()
for i,g in enumerate(r):
 text=f"{g['t']} 41 5\n"+''.join(f'{a} {v}\n' for a,v in g['chords'])
 s=subprocess.run([str(b/'exact_cover'),'30'],input=text,text=True,capture_output=True,check=True)
