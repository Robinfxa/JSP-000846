...
namespace JSP846AutoLarge.C213
open JSP846Standard
open JSP846Replay
def chords : Finset (Sym2 (Fin 9)) := {s(0,1),s(2,4),s(3,5),s(5,7),s(6,8)}
...
theorem z10_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9) (reconstructArcs z10.2 z10.1.val) z10.2 := by
  ...
  all_goals decide +kernel
def z11 : Fin 2 × Finset (Sym2 (Fin 9)) := ((0:Fin 2), {s(2,4),s(3,5),s(6,8)})
...
theorem z11_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9) (reconstructArcs z11.2 z11.1.val) z11.2 := by
  ...
  all_goals decide +kernel
def z12 : Fin 2 × Finset (Sym2 (Fin 9)) := ((1:Fin 2), {s(0,1),s(2,4),s(3,5),s(6,8)})
...
theorem z13_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9) (reconstructArcs z13.2 z13.1.val) z13.2 := by
  ...
  all_goals decide +kernel
def z14 : Fin 2 × Finset (Sym2 (Fin 9)) := ((0:Fin 2), {s(2,4),s(5,7),s(6,8)})
def C14 : Finset (Fin 9) := {2,3,4}
theorem z14_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9) (reconstructArcs z14.2 z14.1.val) z14.2 := by
  ...
  all_goals decide +kernel
def z15 : Fin 2 × Finset (Sym2 (Fin 9)) := ((1:Fin 2), {s(0,1),s(2,4),s(5,7),s(6,8)})
...
theorem table_exact : prunedForms chords bad = JSP846DAG.C213.originalForms.toFinset := by
  decide +kernel
theorem table_subset : encodedSelections (by decide : 0 < 9) chords ⊆ JSP846DAG.C213.originalForms.toFinset := by
  intro f hf
  have hp := encoded_subset_pruned_forms (by decide : 0 < 9) (by decide : 2 ≤ 9) chords bad bad_not_cycle hf
  rw [table_exact] at hp
  exact hp
theorem to_old : ModelFeasible (by decide : 0 < 9) chords → JSP846DAG.C213.Feasible := by
  rintro ⟨w,hw,hcover⟩
  obtain ⟨hpos,hsum,hlower⟩ := hw
  let W : Nat → Nat := extendFinWeight w
  have hc : Covers (values W JSP846DAG.C213.originalForms) :=
    model_literal_cover (by decide : 0 < 9) chords JSP846DAG.C213.originalForms w table_subset hcover
...
theorem excluded : ¬ ModelFeasible (by decide : 0 < 9) chords := by
  intro h
  exact JSP846DAG.C213.infeasible (to_old h)
end JSP846AutoLarge.C213
#print axioms JSP846AutoLarge.C213.table_exact
#print axioms JSP846AutoLarge.C213.excluded
