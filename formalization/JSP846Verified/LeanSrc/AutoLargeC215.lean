...
namespace JSP846AutoLarge.C215
open JSP846Standard
open JSP846Replay
def chords : Finset (Sym2 (Fin 9)) := {s(0,1),s(2,4),s(3,6),s(4,7),s(5,8)}
...
theorem z3_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9) (reconstructArcs z3.2 z3.1.val) z3.2 := by
  ...
  all_goals decide +kernel
def z4 : Fin 2 × Finset (Sym2 (Fin 9)) := ((1:Fin 2), {s(0,1),s(3,6),s(4,7)})
...
theorem z4_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9) (reconstructArcs z4.2 z4.1.val) z4.2 := by
  ...
  all_goals decide +kernel
def z5 : Fin 2 × Finset (Sym2 (Fin 9)) := ((1:Fin 2), {s(0,1),s(2,4),s(3,6),s(4,7)})
...
theorem z10_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9) (reconstructArcs z10.2 z10.1.val) z10.2 := by
  ...
  all_goals decide +kernel
def z11 : Fin 2 × Finset (Sym2 (Fin 9)) := ((1:Fin 2), {s(0,1),s(4,7),s(5,8)})
...
theorem z12_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9) (reconstructArcs z12.2 z12.1.val) z12.2 := by
  ...
  all_goals decide +kernel
def z13 : Fin 2 × Finset (Sym2 (Fin 9)) := ((1:Fin 2), {s(0,1),s(2,4),s(3,6),s(4,7),s(5,8)})
...
theorem table_exact : prunedForms chords bad = JSP846DAG.C215.originalForms.toFinset := by
  decide +kernel
theorem table_subset : encodedSelections (by decide : 0 < 9) chords ⊆ JSP846DAG.C215.originalForms.toFinset := by
  intro f hf
  have hp := encoded_subset_pruned_forms (by decide : 0 < 9) (by decide : 2 ≤ 9) chords bad bad_not_cycle hf
  rw [table_exact] at hp
  exact hp
theorem to_old : ModelFeasible (by decide : 0 < 9) chords → JSP846DAG.C215.Feasible := by
  rintro ⟨w,hw,hcover⟩
  obtain ⟨hpos,hsum,hlower⟩ := hw
  let W : Nat → Nat := extendFinWeight w
  have hc : Covers (values W JSP846DAG.C215.originalForms) :=
    model_literal_cover (by decide : 0 < 9) chords JSP846DAG.C215.originalForms w table_subset hcover
...
theorem excluded : ¬ ModelFeasible (by decide : 0 < 9) chords := by
  intro h
  exact JSP846DAG.C215.infeasible (to_old h)
end JSP846AutoLarge.C215
#print axioms JSP846AutoLarge.C215.table_exact
#print axioms JSP846AutoLarge.C215.excluded
