...
namespace JSP846SevenLarge.C032P
open JSP846Standard
open JSP846Replay
def chords : Finset (Sym2 (Fin 7)) := {s(0,2),s(0,3),s(1,5),s(3,6),s(4,6)}
...
theorem z0_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 7) (reconstructArcs z0.2 z0.1.val) z0.2 := by
  apply symbolic_cut_contradiction (by decide : 0 < 7) (reconstructArcs z0.2 z0.1.val) z0.2 C0 (0:Fin 7) (3:Fin 7)
...
theorem z1_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 7) (reconstructArcs z1.2 z1.1.val) z1.2 := by
  apply symbolic_cut_contradiction (by decide : 0 < 7) (reconstructArcs z1.2 z1.1.val) z1.2 C1 (0:Fin 7) (4:Fin 7)
  all_goals decide +kernel
...
theorem z3_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 7) (reconstructArcs z3.2 z3.1.val) z3.2 := by
  apply symbolic_cut_contradiction (by decide : 0 < 7) (reconstructArcs z3.2 z3.1.val) z3.2 C3 (0:Fin 7) (4:Fin 7)
  all_goals decide +kernel
def z4 : Fin 2 × Finset (Sym2 (Fin 7)) := ((1:Fin 2), {s(0,2),s(3,6),s(4,6)})
...
theorem table_exact : prunedForms chords bad = JSP846DAG.C032.originalForms.toFinset := by
  decide +kernel
theorem table_subset : encodedSelections (by decide : 0 < 7) chords ⊆ JSP846DAG.C032.originalForms.toFinset := by
  intro f hf
  have hp := encoded_subset_pruned_forms (by decide : 0 < 7) (by decide : 2 ≤ 7) chords bad bad_not_cycle hf
  rw [table_exact] at hp
  exact hp
theorem to_old : ModelFeasible (by decide : 0 < 7) chords → JSP846DAG.C032.Feasible := by
  rintro ⟨w,hw,hcover⟩
  obtain ⟨hpos,hsum,hlower⟩ := hw
  let W : Nat → Nat := extendFinWeight w
  have hc : Covers (values W JSP846DAG.C032.originalForms) := model_literal_cover (by decide) chords JSP846DAG.C032.originalForms w table_subset hcover
...
theorem excluded : ¬ ModelFeasible (by decide : 0 < 7) chords := by
  intro h
  exact JSP846DAG.C032.infeasible (to_old h)
end JSP846SevenLarge.C032P
#print axioms JSP846SevenLarge.C032P.table_exact
#print axioms JSP846SevenLarge.C032P.excluded
