...
namespace JSP846EightLarge.C143P
open JSP846Standard
open JSP846Replay
def chords : Finset (Sym2 (Fin 8)) := {s(0,2),s(0,5),s(1,4),s(1,6),s(3,7)}
...
theorem table_exact : prunedForms chords bad = JSP846DAG.C143.originalForms.toFinset := by decide +kernel
theorem table_subset : encodedSelections (by decide : 0 < 8) chords ⊆ JSP846DAG.C143.originalForms.toFinset := by
  intro f hf
  have hp := encoded_subset_pruned_forms (by decide : 0 < 8) (by decide : 2 ≤ 8) chords bad bad_not_cycle hf
  rw [table_exact] at hp
  exact hp
theorem to_old : ModelFeasible (by decide : 0 < 8) chords → JSP846DAG.C143.Feasible := by
  rintro ⟨w,hw,hcover⟩
  obtain ⟨hpos,hsum,hlower⟩ := hw
  let W : Nat → Nat := extendFinWeight w
  have hc : Covers (values W JSP846DAG.C143.originalForms) := model_literal_cover (by decide) chords JSP846DAG.C143.originalForms w table_subset hcover
...
theorem excluded : ¬ ModelFeasible (by decide : 0 < 8) chords := by intro h; exact JSP846DAG.C143.infeasible (to_old h)
end JSP846EightLarge.C143P
#print axioms JSP846EightLarge.C143P.table_exact
#print axioms JSP846EightLarge.C143P.excluded
