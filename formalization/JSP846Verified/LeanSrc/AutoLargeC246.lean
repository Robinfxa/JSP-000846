...
namespace JSP846AutoLarge.C246
open JSP846Standard
open JSP846Replay
def chords : Finset (Sym2 (Fin 9)) := {s(0,2),s(0,6),s(1,4),s(3,8),s(5,7)}
...
theorem z4_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9) (reconstructArcs z4.2 z4.1.val) z4.2 := by
  ...
  all_goals decide +kernel
def z5 : Fin 2 × Finset (Sym2 (Fin 9)) := ((0:Fin 2), {s(0,6),s(1,4),s(5,7)})
def C5 : Finset (Fin 9) := {0,5,6,7,8}
theorem z5_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9) (reconstructArcs z5.2 z5.1.val) z5.2 := by
  ...
  all_goals decide +kernel
def z6 : Fin 2 × Finset (Sym2 (Fin 9)) := ((1:Fin 2), {s(3,8),s(5,7)})
...
theorem z6_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9) (reconstructArcs z6.2 z6.1.val) z6.2 := by
  ...
  all_goals decide +kernel
def z7 : Fin 2 × Finset (Sym2 (Fin 9)) := ((0:Fin 2), {s(0,2),s(3,8),s(5,7)})
...
theorem z7_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9) (reconstructArcs z7.2 z7.1.val) z7.2 := by
  ...
  all_goals decide +kernel
def z8 : Fin 2 × Finset (Sym2 (Fin 9)) := ((1:Fin 2), {s(0,2),s(3,8),s(5,7)})
...
theorem z8_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9) (reconstructArcs z8.2 z8.1.val) z8.2 := by
  ...
  all_goals decide +kernel
def z9 : Fin 2 × Finset (Sym2 (Fin 9)) := ((1:Fin 2), {s(1,4),s(3,8),s(5,7)})
def C9 : Finset (Fin 9) := {0,1,3,4,8}
theorem z9_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9) (reconstructArcs z9.2 z9.1.val) z9.2 := by
  ...
  all_goals decide +kernel
def z10 : Fin 2 × Finset (Sym2 (Fin 9)) := ((0:Fin 2), {s(0,2),s(1,4),s(3,8),s(5,7)})
...
theorem table_exact : prunedForms chords bad = JSP846DAG.C246.originalForms.toFinset := by
  decide +kernel
theorem table_subset : encodedSelections (by decide : 0 < 9) chords ⊆ JSP846DAG.C246.originalForms.toFinset := by
  intro f hf
  have hp := encoded_subset_pruned_forms (by decide : 0 < 9) (by decide : 2 ≤ 9) chords bad bad_not_cycle hf
  rw [table_exact] at hp
  exact hp
theorem to_old : ModelFeasible (by decide : 0 < 9) chords → JSP846DAG.C246.Feasible := by
  rintro ⟨w,hw,hcover⟩
  obtain ⟨hpos,hsum,hlower⟩ := hw
  let W : Nat → Nat := extendFinWeight w
  have hc : Covers (values W JSP846DAG.C246.originalForms) :=
    model_literal_cover (by decide : 0 < 9) chords JSP846DAG.C246.originalForms w table_subset hcover
...
theorem excluded : ¬ ModelFeasible (by decide : 0 < 9) chords := by
  intro h
  exact JSP846DAG.C246.infeasible (to_old h)
end JSP846AutoLarge.C246
#print axioms JSP846AutoLarge.C246.table_exact
#print axioms JSP846AutoLarge.C246.excluded
