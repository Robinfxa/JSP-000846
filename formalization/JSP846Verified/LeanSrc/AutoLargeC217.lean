...
namespace JSP846AutoLarge.C217
open JSP846Standard
open JSP846Replay
def chords : Finset (Sym2 (Fin 9)) := {s(0,1),s(2,4),s(3,7),s(4,6),s(5,8)}
...
theorem z4_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9) (reconstructArcs z4.2 z4.1.val) z4.2 := by
  ...
  all_goals decide +kernel
def z5 : Fin 2 × Finset (Sym2 (Fin 9)) := ((0:Fin 2), {s(0,1),s(3,7),s(4,6)})
...
theorem z5_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9) (reconstructArcs z5.2 z5.1.val) z5.2 := by
  ...
  all_goals decide +kernel
def z6 : Fin 2 × Finset (Sym2 (Fin 9)) := ((1:Fin 2), {s(0,1),s(3,7),s(4,6)})
...
theorem z6_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9) (reconstructArcs z6.2 z6.1.val) z6.2 := by
  ...
  all_goals decide +kernel
def z7 : Fin 2 × Finset (Sym2 (Fin 9)) := ((1:Fin 2), {s(0,1),s(2,4),s(3,7),s(4,6)})
...
theorem z14_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9) (reconstructArcs z14.2 z14.1.val) z14.2 := by
  ...
  all_goals decide +kernel
def z15 : Fin 2 × Finset (Sym2 (Fin 9)) := ((1:Fin 2), {s(0,1),s(2,4),s(3,7),s(4,6),s(5,8)})
...
theorem table_exact : prunedForms chords bad = JSP846DAG.C217.originalForms.toFinset := by
  decide +kernel
theorem table_subset : encodedSelections (by decide : 0 < 9) chords ⊆ JSP846DAG.C217.originalForms.toFinset := by
  intro f hf
  have hp := encoded_subset_pruned_forms (by decide : 0 < 9) (by decide : 2 ≤ 9) chords bad bad_not_cycle hf
  rw [table_exact] at hp
  exact hp
theorem to_old : ModelFeasible (by decide : 0 < 9) chords → JSP846DAG.C217.Feasible := by
  rintro ⟨w,hw,hcover⟩
  obtain ⟨hpos,hsum,hlower⟩ := hw
  let W : Nat → Nat := extendFinWeight w
  have hc : Covers (values W JSP846DAG.C217.originalForms) :=
    model_literal_cover (by decide : 0 < 9) chords JSP846DAG.C217.originalForms w table_subset hcover
...
theorem excluded : ¬ ModelFeasible (by decide : 0 < 9) chords := by
  intro h
  exact JSP846DAG.C217.infeasible (to_old h)
end JSP846AutoLarge.C217
#print axioms JSP846AutoLarge.C217.table_exact
#print axioms JSP846AutoLarge.C217.excluded
