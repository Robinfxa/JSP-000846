import PrunedForms
import MicroC253_098
import Mathlib.Algebra.BigOperators.Fin
set_option autoImplicit false
set_option maxRecDepth 100000
set_option maxHeartbeats 12000000
namespace JSP846AutoLarge.C253
open JSP846Standard
open JSP846Replay
def chords : Finset (Sym2 (Fin 9)) := {s(0,2),s(0,7),s(1,5),s(3,6),s(4,8)}
...
theorem z2_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9) (reconstructArcs z2.2 z2.1.val) z2.2 := by
  ...
  all_goals decide +kernel
def z3 : Fin 2 × Finset (Sym2 (Fin 9)) := ((0:Fin 2), {s(0,7),s(1,5),s(3,6)})
...
theorem z4_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9) (reconstructArcs z4.2 z4.1.val) z4.2 := by
  ...
  all_goals decide +kernel
def z5 : Fin 2 × Finset (Sym2 (Fin 9)) := ((1:Fin 2), {s(0,2),s(3,6),s(4,8)})
...
theorem table_exact : prunedForms chords bad = JSP846DAG.C253.originalForms.toFinset := by
  decide +kernel
theorem table_subset : encodedSelections (by decide : 0 < 9) chords ⊆ JSP846DAG.C253.originalForms.toFinset := by
  intro f hf
  have hp := encoded_subset_pruned_forms (by decide : 0 < 9) (by decide : 2 ≤ 9) chords bad bad_not_cycle hf
  rw [table_exact] at hp
  exact hp
theorem to_old : ModelFeasible (by decide : 0 < 9) chords → JSP846DAG.C253.Feasible := by
  rintro ⟨w,hw,hcover⟩
  obtain ⟨hpos,hsum,hlower⟩ := hw
  let W : Nat → Nat := extendFinWeight w
  have hc : Covers (values W JSP846DAG.C253.originalForms) :=
    model_literal_cover (by decide : 0 < 9) chords JSP846DAG.C253.originalForms w table_subset hcover
...
theorem excluded : ¬ ModelFeasible (by decide : 0 < 9) chords := by
  intro h
  exact JSP846DAG.C253.infeasible (to_old h)
end JSP846AutoLarge.C253
#print axioms JSP846AutoLarge.C253.table_exact
#print axioms JSP846AutoLarge.C253.excluded
