...
namespace JSP846EightLarge.C167P
open JSP846Standard
open JSP846Replay
def chords : Finset (Sym2 (Fin 8)) := {s(0,3),s(0,4),s(1,5),s(1,6),s(2,7)}
def z0 : Fin 2 × Finset (Sym2 (Fin 8)) := ((0:Fin 2), {s(0,3),s(0,4),s(1,5),s(1,6)})
...
theorem z1_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 8) (reconstructArcs z1.2 z1.1.val) z1.2 := by
  ...
  all_goals decide +kernel
def z2 : Fin 2 × Finset (Sym2 (Fin 8)) := ((0:Fin 2), {s(0,3),s(0,4),s(1,6),s(2,7)})
...
theorem z2_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 8) (reconstructArcs z2.2 z2.1.val) z2.2 := by
  ...
  all_goals decide +kernel
def z3 : Fin 2 × Finset (Sym2 (Fin 8)) := ((0:Fin 2), {s(0,3),s(1,5),s(1,6),s(2,7)})
...
theorem z3_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 8) (reconstructArcs z3.2 z3.1.val) z3.2 := by
  ...
  all_goals decide +kernel
def z4 : Fin 2 × Finset (Sym2 (Fin 8)) := ((0:Fin 2), {s(0,4),s(1,5),s(1,6),s(2,7)})
...
theorem table_exact : prunedForms chords bad = JSP846DAG.C167.originalForms.toFinset := by decide +kernel
theorem table_subset : encodedSelections (by decide : 0 < 8) chords ⊆ JSP846DAG.C167.originalForms.toFinset := by
  intro f hf
  have hp := encoded_subset_pruned_forms (by decide : 0 < 8) (by decide : 2 ≤ 8) chords bad bad_not_cycle hf
  rw [table_exact] at hp
  exact hp
theorem to_old : ModelFeasible (by decide : 0 < 8) chords → JSP846DAG.C167.Feasible := by
  rintro ⟨w,hw,hcover⟩
  obtain ⟨hpos,hsum,hlower⟩ := hw
  let W : Nat → Nat := extendFinWeight w
  have hc : Covers (values W JSP846DAG.C167.originalForms) := model_literal_cover (by decide) chords JSP846DAG.C167.originalForms w table_subset hcover
...
theorem excluded : ¬ ModelFeasible (by decide : 0 < 8) chords := by intro h; exact JSP846DAG.C167.infeasible (to_old h)
end JSP846EightLarge.C167P
#print axioms JSP846EightLarge.C167P.table_exact
#print axioms JSP846EightLarge.C167P.excluded
