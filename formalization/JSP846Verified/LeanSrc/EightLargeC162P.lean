...
namespace JSP846EightLarge.C162P
open JSP846Standard
open JSP846Replay
def chords : Finset (Sym2 (Fin 8)) := {s(0,2),s(1,4),s(1,5),s(3,6),s(3,7)}
...
def z1 : Fin 2 × Finset (Sym2 (Fin 8)) := ((0:Fin 2), {s(0,2),s(1,4),s(1,5),s(3,6)})
def C1 : Finset (Fin 8) := {0,2,3,6,7}
...
theorem z2_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 8) (reconstructArcs z2.2 z2.1.val) z2.2 := by
  ...
  all_goals decide +kernel
def z3 : Fin 2 × Finset (Sym2 (Fin 8)) := ((0:Fin 2), {s(0,2),s(1,4),s(1,5),s(3,7)})
...
theorem z3_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 8) (reconstructArcs z3.2 z3.1.val) z3.2 := by
  ...
  all_goals decide +kernel
def z4 : Fin 2 × Finset (Sym2 (Fin 8)) := ((1:Fin 2), {s(0,2),s(3,6),s(3,7)})
...
theorem z4_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 8) (reconstructArcs z4.2 z4.1.val) z4.2 := by
  ...
  all_goals decide +kernel
def z5 : Fin 2 × Finset (Sym2 (Fin 8)) := ((0:Fin 2), {s(1,4),s(1,5),s(3,6),s(3,7)})
def C5 : Finset (Fin 8) := {1,4,5}
theorem z5_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 8) (reconstructArcs z5.2 z5.1.val) z5.2 := by
  apply symbolic_cut_contradiction (by decide : 0 < 8) (reconstructArcs z5.2 z5.1.val) z5.2 C5 (1:Fin 8) (3:Fin 8)
  all_goals decide +kernel
...
theorem table_exact : prunedForms chords bad = JSP846DAG.C162.originalForms.toFinset := by decide +kernel
theorem table_subset : encodedSelections (by decide : 0 < 8) chords ⊆ JSP846DAG.C162.originalForms.toFinset := by
  intro f hf
  have hp := encoded_subset_pruned_forms (by decide : 0 < 8) (by decide : 2 ≤ 8) chords bad bad_not_cycle hf
  rw [table_exact] at hp
  exact hp
theorem to_old : ModelFeasible (by decide : 0 < 8) chords → JSP846DAG.C162.Feasible := by
  rintro ⟨w,hw,hcover⟩
  obtain ⟨hpos,hsum,hlower⟩ := hw
  let W : Nat → Nat := extendFinWeight w
  have hc : Covers (values W JSP846DAG.C162.originalForms) := model_literal_cover (by decide) chords JSP846DAG.C162.originalForms w table_subset hcover
...
theorem excluded : ¬ ModelFeasible (by decide : 0 < 8) chords := by intro h; exact JSP846DAG.C162.infeasible (to_old h)
end JSP846EightLarge.C162P
#print axioms JSP846EightLarge.C162P.table_exact
#print axioms JSP846EightLarge.C162P.excluded
