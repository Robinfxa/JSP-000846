...
namespace JSP846EightLarge.C126P
open JSP846Standard
open JSP846Replay
def chords : Finset (Sym2 (Fin 8)) := {s(0,2),s(0,4),s(1,5),s(1,7),s(3,6)}
def z0 : Fin 2 × Finset (Sym2 (Fin 8)) := ((0:Fin 2), {s(0,2),s(0,4),s(1,5),s(1,7)})
def C0 : Finset (Fin 8) := {0,2,3,4}
...
theorem table_exact : prunedForms chords bad = JSP846DAG.C126.originalForms.toFinset := by decide +kernel
theorem table_subset : encodedSelections (by decide : 0 < 8) chords ⊆ JSP846DAG.C126.originalForms.toFinset := by
  intro f hf
  have hp := encoded_subset_pruned_forms (by decide : 0 < 8) (by decide : 2 ≤ 8) chords bad bad_not_cycle hf
  rw [table_exact] at hp
  exact hp
theorem to_old : ModelFeasible (by decide : 0 < 8) chords → JSP846DAG.C126.Feasible := by
  rintro ⟨w,hw,hcover⟩
  obtain ⟨hpos,hsum,hlower⟩ := hw
  let W : Nat → Nat := extendFinWeight w
  have hc : Covers (values W JSP846DAG.C126.originalForms) := model_literal_cover (by decide) chords JSP846DAG.C126.originalForms w table_subset hcover
...
theorem excluded : ¬ ModelFeasible (by decide : 0 < 8) chords := by intro h; exact JSP846DAG.C126.infeasible (to_old h)
end JSP846EightLarge.C126P
#print axioms JSP846EightLarge.C126P.table_exact
#print axioms JSP846EightLarge.C126P.excluded
