...
def nineDecodeNat (S : Finset Nat) : Finset (Sym2 (Fin 9)) :=
  S.image (fun n => if h : n < 36 then nineEdgeFin ⟨n,h⟩ else s(0,1))

theorem nineDecodeNat_toNat (I : Finset (Fin 36)) :
    nineDecodeNat (nineToNat I) = nineDecodeFin I := by
  ext d
  constructor
  · intro hd
...
#print axioms JSP846Standard.nineDecodeNat_toNat
