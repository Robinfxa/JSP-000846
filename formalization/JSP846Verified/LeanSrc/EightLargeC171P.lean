...
namespace JSP846EightLarge.C171P
open JSP846Standard
open JSP846Replay
def chords : Finset (Sym2 (Fin 8)) := {s(0,3),s(0,4),s(1,5),s(2,7),s(3,6)}
def z0 : Fin 2 × Finset (Sym2 (Fin 8)) := ((0:Fin 2), {s(0,3),s(0,4),s(1,5),s(2,7)})
...
def z1 : Fin 2 × Finset (Sym2 (Fin 8)) := ((1:Fin 2), {s(2,7),s(3,6)})
def C1 : Finset (Fin 8) := {0,1,2,7}
theorem z1_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 8) (reconstructArcs z1.2 z1.1.val) z1.2 := by
  ...
  all_goals decide +kernel
def z2 : Fin 2 × Finset (Sym2 (Fin 8)) := ((1:Fin 2), {s(0,4),s(1,5),s(2,7),s(3,6)})
...
theorem table_exact : prunedForms chords bad = JSP846DAG.C171.originalForms.toFinset := by decide +kernel
theorem table_subset : encodedSelections (by decide : 0 < 8) chords ⊆ JSP846DAG.C171.originalForms.toFinset := by
  intro f hf
  have hp := encoded_subset_pruned_forms (by decide : 0 < 8) (by decide : 2 ≤ 8) chords bad bad_not_cycle hf
  rw [table_exact] at hp
  exact hp
theorem to_old : ModelFeasible (by decide : 0 < 8) chords → JSP846DAG.C171.Feasible := by
  rintro ⟨w,hw,hcover⟩
  obtain ⟨hpos,hsum,hlower⟩ := hw
  let W : Nat → Nat := extendFinWeight w
  have hc : Covers (values W JSP846DAG.C171.originalForms) := model_literal_cover (by decide) chords JSP846DAG.C171.originalForms w table_subset hcover
...
theorem excluded : ¬ ModelFeasible (by decide : 0 < 8) chords := by intro h; exact JSP846DAG.C171.infeasible (to_old h)
end JSP846EightLarge.C171P
#print axioms JSP846EightLarge.C171P.table_exact
#print axioms JSP846EightLarge.C171P.excluded
