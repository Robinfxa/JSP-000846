...
namespace JSP846SevenLarge.C040P
open JSP846Standard
open JSP846Replay
def chords : Finset (Sym2 (Fin 7)) := {s(0,2),s(0,4),s(1,4),s(2,5),s(3,6)}
...
def z1 : Fin 2 × Finset (Sym2 (Fin 7)) := ((1:Fin 2), {s(0,4),s(1,4),s(2,5),s(3,6)})
def C1 : Finset (Fin 7) := {0,1,4}
...
theorem table_exact : prunedForms chords bad = JSP846DAG.C040.originalForms.toFinset := by
  decide +kernel
theorem table_subset : encodedSelections (by decide : 0 < 7) chords ⊆ JSP846DAG.C040.originalForms.toFinset := by
  intro f hf
  have hp := encoded_subset_pruned_forms (by decide : 0 < 7) (by decide : 2 ≤ 7) chords bad bad_not_cycle hf
  rw [table_exact] at hp
  exact hp
theorem to_old : ModelFeasible (by decide : 0 < 7) chords → JSP846DAG.C040.Feasible := by
  rintro ⟨w,hw,hcover⟩
  obtain ⟨hpos,hsum,hlower⟩ := hw
  let W : Nat → Nat := extendFinWeight w
  have hc : Covers (values W JSP846DAG.C040.originalForms) := model_literal_cover (by decide) chords JSP846DAG.C040.originalForms w table_subset hcover
...
theorem excluded : ¬ ModelFeasible (by decide : 0 < 7) chords := by
  intro h
  exact JSP846DAG.C040.infeasible (to_old h)
end JSP846SevenLarge.C040P
#print axioms JSP846SevenLarge.C040P.table_exact
#print axioms JSP846SevenLarge.C040P.excluded
