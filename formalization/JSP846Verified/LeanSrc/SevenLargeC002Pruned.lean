...
namespace JSP846SevenLarge.C002P
open JSP846Standard
open JSP846Replay

def chords : Finset (Sym2 (Fin 7)) :=
  {s(0,1),s(0,2),s(1,4),s(2,5),s(3,6)}

def z0 : Fin 2 × Finset (Sym2 (Fin 7)) := ((1:Fin 2), {s(0,1),s(2,5)})
def C0 : Finset (Fin 7) := {0,1}
theorem z0_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 7)
...
theorem z1_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 7)
...
theorem z2_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 7)
...
theorem z3_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 7)
...
theorem z4_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 7)
...
theorem bad_not_cycle : ∀ z ∈ bad,
    ¬ IsSymbolicCycle (by decide : 0 < 7) (reconstructArcs z.2 z.1.val) z.2 := by
  intro z hz
...
theorem table_exact :
    prunedForms chords bad = JSP846DAG.C002.originalForms.toFinset := by
  decide +kernel

theorem table_subset :
    encodedSelections (by decide : 0 < 7) chords ⊆
      JSP846DAG.C002.originalForms.toFinset := by
  intro f hf
  have hp := encoded_subset_pruned_forms (by decide : 0 < 7) (by decide : 2 ≤ 7)
    chords bad bad_not_cycle hf
  rw [table_exact] at hp
  exact hp

theorem to_old : ModelFeasible (by decide : 0 < 7) chords →
    JSP846DAG.C002.Feasible := by
  rintro ⟨w,hw,hcover⟩
  obtain ⟨hpos,hsum,hlower⟩ := hw
  let W : Nat → Nat := extendFinWeight w
  have hc : Covers (values W JSP846DAG.C002.originalForms) :=
    model_literal_cover (by decide) chords JSP846DAG.C002.originalForms w
      table_subset hcover
...
theorem excluded : ¬ ModelFeasible (by decide : 0 < 7) chords := by
  intro h
  exact JSP846DAG.C002.infeasible (to_old h)

end JSP846SevenLarge.C002P
#print axioms JSP846SevenLarge.C002P.table_exact
#print axioms JSP846SevenLarge.C002P.excluded
