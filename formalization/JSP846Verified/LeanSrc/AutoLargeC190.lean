...
namespace JSP846AutoLarge.C190
open JSP846Standard
open JSP846Replay
def chords : Finset (Sym2 (Fin 9)) := {s(0,1),s(0,5),s(2,6),s(3,8),s(4,7)}
...
theorem z8_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9) (reconstructArcs z8.2 z8.1.val) z8.2 := by
  ...
  all_goals decide +kernel
def z9 : Fin 2 × Finset (Sym2 (Fin 9)) := ((1:Fin 2), {s(0,5),s(2,6),s(3,8),s(4,7)})
...
theorem z9_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9) (reconstructArcs z9.2 z9.1.val) z9.2 := by
  ...
  all_goals decide +kernel
def z10 : Fin 2 × Finset (Sym2 (Fin 9)) := ((0:Fin 2), {s(0,1),s(0,5),s(2,6),s(3,8),s(4,7)})
...
theorem table_exact : prunedForms chords bad = JSP846DAG.C190.originalForms.toFinset := by
  decide +kernel
theorem table_subset : encodedSelections (by decide : 0 < 9) chords ⊆ JSP846DAG.C190.originalForms.toFinset := by
  intro f hf
  have hp := encoded_subset_pruned_forms (by decide : 0 < 9) (by decide : 2 ≤ 9) chords bad bad_not_cycle hf
  rw [table_exact] at hp
  exact hp
theorem to_old : ModelFeasible (by decide : 0 < 9) chords → JSP846DAG.C190.Feasible := by
  rintro ⟨w,hw,hcover⟩
  obtain ⟨hpos,hsum,hlower⟩ := hw
  let W : Nat → Nat := extendFinWeight w
  have hc : Covers (values W JSP846DAG.C190.originalForms) :=
    model_literal_cover (by decide : 0 < 9) chords JSP846DAG.C190.originalForms w table_subset hcover
...
theorem excluded : ¬ ModelFeasible (by decide : 0 < 9) chords := by
  intro h
  exact JSP846DAG.C190.infeasible (to_old h)
end JSP846AutoLarge.C190
#print axioms JSP846AutoLarge.C190.table_exact
#print axioms JSP846AutoLarge.C190.excluded
