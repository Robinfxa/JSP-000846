...
namespace JSP846NineLarge.C276P
open JSP846Standard
open JSP846Replay
open JSP846HintBridge
open JSP846Residual
def chords : Finset (Sym2 (Fin 9)) := {s(0,3),s(0,6),s(1,5),s(2,7),s(4,8)}
...
def z1 : Fin 2 × Finset (Sym2 (Fin 9)) := ((0:Fin 2), {s(0,3),s(1,5),s(2,7),s(4,8)})
...
theorem z2_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9)
  ...
  all_goals decide +kernel
def z3 : Fin 2 × Finset (Sym2 (Fin 9)) := ((1:Fin 2), {s(0,6),s(1,5),s(2,7),s(4,8)})
def C3 : Finset (Fin 9) := {0,1,5,6}
theorem z3_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 9)
  ...
  all_goals decide +kernel
def bad : Finset (Fin 2 × Finset (Sym2 (Fin 9))) := {z0,z1,z2,z3}
...
def replayForms : List JSP846Replay.Form :=
  JSP846Residual.HintCases.C276.forms.map JSP846HintBridge.exprToForm
...
theorem excluded : ¬ ModelFeasible (by decide : 0 < 9) chords := by
  ...
  have hb : JSP846Residual.Bounds JSP846Residual.HintCases.C276.lower xs := by
    simp [JSP846Residual.HintCases.C276.lower, xs, JSP846HintBridge.finList9,
  ...
  have hc' : JSP846Replay.Covers
      (JSP846Replay.values (JSP846HintBridge.listWeight xs)
        (JSP846Residual.HintCases.C276.forms.map JSP846HintBridge.exprToForm)) := by
    have hwEq := JSP846HintBridge.listWeight_finList9 w
    rw [hwEq]
    simpa [replayForms] using hc
  have ho : JSP846Residual.OriginalFeasible
      JSP846Residual.HintCases.C276.lower
      JSP846Residual.HintCases.C276.forms :=
    JSP846HintBridge.originalFeasible_of_replay
      JSP846Residual.HintCases.C276.lower
      JSP846Residual.HintCases.C276.forms xs hb hs hc'
  exact JSP846Residual.HintCases.C276.infeasible ho
end JSP846NineLarge.C276P
#print axioms JSP846NineLarge.C276P.pruned_subset
#print axioms JSP846NineLarge.C276P.excluded
