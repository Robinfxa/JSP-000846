...
namespace JSP846TenAutoHint.C315
open JSP846Standard
open JSP846Replay
open JSP846HintBridge
open JSP846Residual
open JSP846TenAutoTable.C315

theorem replay_eq_hint :
    replayForms = JSP846Residual.HintCases.C315.forms.map JSP846HintBridge.exprToForm := by
  decide +kernel
...
theorem excluded : ¬ ModelFeasible (by decide : 0 < 10) chords := by
  ...
  have hb : JSP846Residual.Bounds JSP846Residual.HintCases.C315.lower xs := by
    simp [JSP846Residual.HintCases.C315.lower, xs, JSP846HintBridge.finList10,
  ...
  have hc' : JSP846Replay.Covers
      (JSP846Replay.values (JSP846HintBridge.listWeight xs)
        (JSP846Residual.HintCases.C315.forms.map JSP846HintBridge.exprToForm)) := by
    have hwEq := JSP846HintBridge.listWeight_finList10 w
    rw [hwEq]
    rw [← replay_eq_hint]
    exact hc
  have ho : JSP846Residual.OriginalFeasible
      JSP846Residual.HintCases.C315.lower
      JSP846Residual.HintCases.C315.forms :=
    JSP846HintBridge.originalFeasible_of_replay
      JSP846Residual.HintCases.C315.lower
      JSP846Residual.HintCases.C315.forms xs hb hs hc'
  exact JSP846Residual.HintCases.C315.infeasible ho

end JSP846TenAutoHint.C315
#print axioms JSP846TenAutoHint.C315.replay_eq_hint
#print axioms JSP846TenAutoHint.C315.excluded
