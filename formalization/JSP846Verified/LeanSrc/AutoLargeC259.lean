...
namespace JSP846AutoLarge.C259
open JSP846Standard
open JSP846Replay
def chords : Finset (Sym2 (Fin 9)) := {s(0,2),s(1,4),s(1,7),s(3,6),s(5,8)}
...
theorem table_exact : prunedForms chords bad = JSP846DAG.C259.originalForms.toFinset := by
  decide +kernel
theorem table_subset : encodedSelections (by decide : 0 < 9) chords ⊆ JSP846DAG.C259.originalForms.toFinset := by
  intro f hf
  have hp := encoded_subset_pruned_forms (by decide : 0 < 9) (by decide : 2 ≤ 9) chords bad bad_not_cycle hf
  rw [table_exact] at hp
  exact hp
theorem to_old : ModelFeasible (by decide : 0 < 9) chords → JSP846DAG.C259.Feasible := by
  rintro ⟨w,hw,hcover⟩
  obtain ⟨hpos,hsum,hlower⟩ := hw
  let W : Nat → Nat := extendFinWeight w
  have hc : Covers (values W JSP846DAG.C259.originalForms) :=
    model_literal_cover (by decide : 0 < 9) chords JSP846DAG.C259.originalForms w table_subset hcover
...
theorem excluded : ¬ ModelFeasible (by decide : 0 < 9) chords := by
  intro h
  exact JSP846DAG.C259.infeasible (to_old h)
end JSP846AutoLarge.C259
#print axioms JSP846AutoLarge.C259.table_exact
#print axioms JSP846AutoLarge.C259.excluded
