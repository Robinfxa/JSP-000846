...
namespace JSP846SevenLarge.C045P
open JSP846Standard
open JSP846Replay
def chords : Finset (Sym2 (Fin 7)) := {s(0,3),s(0,4),s(1,4),s(2,5),s(2,6)}
def z0 : Fin 2 × Finset (Sym2 (Fin 7)) := ((0:Fin 2), {s(0,3),s(0,4),s(2,5),s(2,6)})
...
def z1 : Fin 2 × Finset (Sym2 (Fin 7)) := ((1:Fin 2), {s(0,3),s(1,4),s(2,5),s(2,6)})
def C1 : Finset (Fin 7) := {0,1,3,4}
theorem z1_not_cycle : ¬ IsSymbolicCycle (by decide : 0 < 7) (reconstructArcs z1.2 z1.1.val) z1.2 := by
  ...
  all_goals decide +kernel
def z2 : Fin 2 × Finset (Sym2 (Fin 7)) := ((1:Fin 2), {s(0,4),s(1,4),s(2,5),s(2,6)})
def C2 : Finset (Fin 7) := {0,1,4}
...
theorem table_exact : prunedForms chords bad = JSP846DAG.C045.originalForms.toFinset := by
  decide +kernel
theorem table_subset : encodedSelections (by decide : 0 < 7) chords ⊆ JSP846DAG.C045.originalForms.toFinset := by
  intro f hf
  have hp := encoded_subset_pruned_forms (by decide : 0 < 7) (by decide : 2 ≤ 7) chords bad bad_not_cycle hf
  rw [table_exact] at hp
  exact hp
theorem to_old : ModelFeasible (by decide : 0 < 7) chords → JSP846DAG.C045.Feasible := by
  rintro ⟨w,hw,hcover⟩
  obtain ⟨hpos,hsum,hlower⟩ := hw
  let W : Nat → Nat := extendFinWeight w
  have hc : Covers (values W JSP846DAG.C045.originalForms) := model_literal_cover (by decide) chords JSP846DAG.C045.originalForms w table_subset hcover
...
theorem excluded : ¬ ModelFeasible (by decide : 0 < 7) chords := by
  intro h
  exact JSP846DAG.C045.infeasible (to_old h)
end JSP846SevenLarge.C045P
#print axioms JSP846SevenLarge.C045P.table_exact
#print axioms JSP846SevenLarge.C045P.excluded
